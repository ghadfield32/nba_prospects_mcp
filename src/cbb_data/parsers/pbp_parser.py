...
def extract_player_mapping(boxscore: dict) -> dict:
    ...
    logger.info(f"Extracted player mapping for {len(player_map)} players from {len(players_array)} teams")
    return player_map
...
def _handle_shot(stats: dict, shooter_id: str, participants: list, score_value: int, text: str) -> None:
...
def parse_game_to_box_score(
    ...
    df['Dorsal'] = df['PLAYER_ID'].map(
        lambda pid: player_mapping.get(pid, {}).get('jersey', '')
    )
    ...
    column_order = [
        'SEASON', 'GAME_ID', 'Home', 'PLAYER_ID', 'STARTER', 'IsPlaying',
        'TEAM', 'Dorsal', 'PLAYER_NAME', 'MIN', 'PTS',
        'FG2M', 'FG2A', 'FG3M', 'FG3A', 'FTM', 'FTA',
        'OREB', 'DREB', 'REB', 'AST', 'STL', 'TOV', 'BLK', 'BLK_AGAINST',
        'PF', 'PF_DRAWN', 'VALUATION', 'PLUS_MINUS', 'LEAGUE', 'FGM', 'FGA', 'FG_PCT'
    ]

    # Ensure all columns exist (add missing ones with default values)
    for col in column_order:
        if col not in df.columns:
            df[col] = 0 if col in ['Home', 'STARTER', 'IsPlaying', 'VALUATION', 'BLK_AGAINST', 'PF_DRAWN'] else ''
