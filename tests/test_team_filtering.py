...
class TestSingleTeamFiltering:
    ...
    def test_single_team_schedule(self) -> None:
        """Test retrieving all games for a single team"""
        print("\n[TEST] Single team schedule retrieval")

        df = get_basketball_data(
            dataset="schedule", league=LEAGUE_NCAA, season=TEST_SEASON, teams=["Duke"]
        )
    ...
    def test_team_player_stats(self) -> None:
    ...
    def test_case_insensitive_team_name(self) -> None:
        """Test that team names are case-insensitive"""
        print("\n[TEST] Case-insensitive team names")

        # Try different cases
        df_lower = get_basketball_data(
            dataset="schedule", league=LEAGUE_NCAA, season=TEST_SEASON, teams=["duke"], limit=5
        )
        ...
        print(
            f"✓ Case insensitivity works: duke={len(df_lower)}, DUKE={len(df_upper)}, Duke={len(df_mixed)}"
        )
...
class TestHeadToHeadFiltering:
    ...
    def test_head_to_head_schedule(self) -> None:
        ...
        if not df.empty:
            ...
            for _, row in df.iterrows():
                ...
                assert (
                    has_duke and has_unc
                ), f"Game should include both teams: {row['HOME_TEAM']} vs {row['AWAY_TEAM']}"
    ...
    def test_head_to_head_player_stats(self) -> None:
        ...
        if not df.empty:
            assert "PLAYER_NAME" in df.columns, "Should have PLAYER_NAME column"
            assert (
                "TEAM_ABBREVIATION" in df.columns or "TEAM_NAME" in df.columns
...
class TestEuroLeagueTeamFiltering:
    ...
    def test_euroleague_team_schedule(self) -> None:
        """Test EuroLeague team schedule retrieval"""
        print("\n[TEST] EuroLeague team schedule")

        df = get_basketball_data(
            dataset="schedule", league=LEAGUE_EURO, season="2024", teams=["Barcelona"], limit=10
        )
    ...
    def test_euroleague_team_player_stats(self) -> None:
...
class TestTeamFilterValidation:
    ...
    def test_empty_teams_list(self) -> None:
        ...
        df = get_basketball_data(
            dataset="schedule", league=LEAGUE_NCAA, season=TEST_SEASON, teams=[], limit=10
        )
    ...
    def test_more_than_two_teams_error(self) -> None:
...
class TestTeamFilterDataQuality:
    ...
    def test_team_schedule_completeness(self) -> None:
        """Test that team schedules have required fields"""
        print("\n[TEST] Team schedule data completeness")

        df = get_basketball_data(
            dataset="schedule", league=LEAGUE_NCAA, season=TEST_SEASON, teams=["Gonzaga"], limit=5
        )
    ...
    def test_team_stats_accuracy(self) -> None:
