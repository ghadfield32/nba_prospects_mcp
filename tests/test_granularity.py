...
class TestHalfLevelAggregation:
    ...
    def test_half_aggregation_basic(self):
        ...
        assert len(df) <= unique_players * 2, f"Should have at most {unique_players * 2} rows (players × halves), got {len(df)}"
        ...
        print(f"✓ Basic structure valid: {len(df)} rows, {unique_players} players (up to 2 halves each)")

    def test_half_filtering(self):
    ...
    def test_second_half_filtering(self):
    ...
    def test_half_stats_accuracy(self):
        ...
        total_pts_aggregated = df.groupby('PLAYER_NAME')['PTS'].sum()
...
class TestQuarterLevelAggregation:
    ...
    @pytest.mark.skip(reason="EuroLeague quarter aggregation not yet implemented - pending PBP data")
    def test_quarter_aggregation_basic(self):
...
class TestPlayLevelData:
    ...
    def test_play_level_retrieval(self):
    ...
    def test_play_level_half_filtering(self):
        ...
        print(f"✓ Play-level filtering works: {len(df_h1)} events in first half (vs {len(df_all)} total)")
...
class TestGranularityValidation:
    ...
    def test_half_with_euroleague_error(self):
    ...
    def test_quarter_with_ncaa_error(self):
...
class TestBackwardCompatibility:
    ...
    def test_explicit_game_granularity(self):
        ...
        for method_name in dir(instance):
            if method_name.startswith('test_'):
                ...
                if hasattr(method, 'pytestmark'):
                    for mark in method.pytestmark if isinstance(method.pytestmark, list) else [method.pytestmark]:
                        if mark.name == 'skip':
                            skip_marker = mark
                            break
