#!/usr/bin/env python3
"""Build combined LNB PBP dataset with content-level validation

This script implements Task #1 from the unified architecture:
- Combines all leagues into one dataset with `league` as column
- Attaches metadata from game index
- Validates content (monotonic scores, valid enums, etc.)
- Quarantines invalid games
- Generates quality report

Usage:
    python tools/lnb/build_lnb_combined_pbp.py --season 2023-2024
    python tools/lnb/build_lnb_combined_pbp.py --season 2023-2024 --force
"""
...
from src.cbb_data.lnb.validation import ValidationResult, validate_pbp_batch

# Paths
INDEX_PATH = Path("data/raw/lnb/lnb_game_index.parquet")
RAW_PBP_DIR = Path("data/raw/lnb/pbp")
CURATED_DIR = Path("data/curated/lnb/pbp")
QUARANTINE_DIR = Path("data/quarantine/lnb/pbp")
REPORTS_DIR = Path("data/curated/lnb/reports")


def load_game_index(season: str) -> pd.DataFrame:
    """Load game index filtered by season and has_pbp=True"""
    if not INDEX_PATH.exists():
        raise FileNotFoundError(f"Game index not found: {INDEX_PATH}")

    df = pd.read_parquet(INDEX_PATH)

    # Filter to season and games with PBP
    df = df[df["season"] == season]
    df = df[df["has_pbp"] == True]  # noqa: E712
...
def load_raw_pbp_game(game_id: str, season: str) -> pd.DataFrame:
    """Load raw PBP data for a single game"""
    pbp_file = RAW_PBP_DIR / f"season={season}" / f"game_id={game_id}.parquet"
...
def attach_metadata(
    pbp_df: pd.DataFrame, game_row: pd.Series, metadata_cols: list[str]
) -> pd.DataFrame:
    """Attach metadata columns from game index to PBP dataframe

    Args:
        pbp_df: Raw PBP dataframe
        game_row: Row from game index
        metadata_cols: List of column names to attach

    Returns:
        PBP dataframe with metadata columns added
    """
    result_df = pbp_df.copy()
...
def write_quarantine(
    ...
    Args:
        ...
        df: PBP dataframe to quarantine
...
def build_combined_pbp(season: str, force: bool = False) -> dict:
    """Build combined PBP dataset for a season
    ...
    """
    print("=" * 80)
    print(f"  BUILD COMBINED LNB PBP: {season}")
    print("=" * 80)
    ...
    print(f"[INFO] Found {len(index_df)} games with PBP data")

    # Load raw PBP for each game
    print("\n[INFO] Loading raw PBP data...")
    games = {}
    load_errors = []
    ...
    for _, game_row in index_df.iterrows():
        ...
        try:
            pbp_df = load_raw_pbp_game(game_id, season)

            # Attach metadata
            pbp_df = attach_metadata(pbp_df, game_row, METADATA_COLUMNS_TO_ATTACH)

            games[game_id] = pbp_df
    ...
    valid_games, validation_results = validate_pbp_batch(games)
    ...
    output_file = output_dir / "lnb_pbp.parquet"
    ...
    report_file = reports_season_dir / "pbp_quality_report.json"
...
def main():
    parser = argparse.ArgumentParser(description="Build combined LNB PBP dataset")
    ...
    try:
        report = build_combined_pbp(args.season, force=args.force)
